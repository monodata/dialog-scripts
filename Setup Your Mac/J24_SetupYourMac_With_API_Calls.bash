...
#
#   Version 1.7.0B, 04-Feb-2023, Chris Jarvis (chris.jarvis@jigsaw24.com)
#   - Branching from Dans version
#   - Stealing script parameters from Dans defaults
#   -
#
####################################################################################################
...
####################################################################################################
#
#   Credentials are hashed and salted to provide increased obfuscation
#   - Salt:             api_salt=$(openssl rand -hex 8)
#   - PassPhrase:       api_phrase=$(openssl rand -hex 12)
#   - Encoded Creds:    api_encCred=$(echo "$pass" | openssl enc -aes256 -a -A -S $api_salt -k $api_phrase)
#
# Hashed & Salted creds passed in script parameters

# api_userCred in format: api_user|U2FsdGVkX1/Sfm1ZRVN5RIUDHyNmOpKtZjHmF0OJlZE=%
api_userCred="$4"

# Split the user and encPass
api_user=$(echo $api_userCred | awk -F'|' '{print $1}')
api_encPass=$(echo $cred | awk -F'|' '{print $2}')

# Catch the salt & PassPhrase
api_salt="$5"
api_phrase="$6"

# Get the raw pass
local api_pass=$(echo "$api_encPass" | /usr/bin/openssl enc -aes256 -d -a -A -S "$api_salt" -k "$api_phrase")

# Create base64-encoded credentials
api_creds=$(printf "$api_user:$api_pass" | /usr/bin/iconv -t ISO-8859-1 | /usr/bin/base64 -i -)

# jamf Pro URL
jamf_url=$(defaults read /Library/Preferences/com.jamfsoftware.jamf.plist jss_url)

# Get the device serial number
serial_number=$(/usr/sbin/system_profiler SPHardwareDataType | grep 'Serial Number (system)' | awk '{print $NF}')
...
function createAPIToken() {
    ...
    authToken=$(/usr/bin/curl \
        --silent \
        --request POST \
        --header "Authorization: Basic ${api_creds}" \
        --header "Accept: application/json" \
        --url "${jamf_url}/api/v1/auth/token")

    api_token=$(echo $authToken | awk '/"token" :/ { print $4 }' | sed 's/"//g;s/,//g')
...
#
# ****These have been updated as the script has been adapted to use the API****
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #

scriptVersion="1.7.0B"
export PATH=/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin/

# Your organization's default location for client-side logs
scriptLog="/var/tmp/com.jigsaw24.SetupMyMac.log"

# [ true | verbose (default) | false ]
debugMode="${8:-"verbose"}"

# Changing this as it will depend on the API result
#welcomeDialog="true"

# Changing this as it will always be an auto-restart
completionActionOption="Restart"

# Your organization's required minimum build of macOS to allow users to proceed
### Wont need this as machines will be built to latest OS
requiredMinimumBuild="22D"

# Jamf Pro Self Service policy for operating system ugprades (i.e., "jamfselfservice://content?entity=policy&id=117&action=view")
outdatedOsAction="/System/Library/CoreServices/Software Update.app"

# Initialize dynamic recon options; built based on user's input at Welcome dialog
reconOptions=""

# Default exit code (i.e., "0" equals sucess)
exitCode="0"
...
echo -e "\n###\n# Setup Your Mac (${scriptVersion})\n# https://github.com/monodata/dialog-scripts/blob/main/Setup%20Your%20Mac/Setup-Your-Mac-via-Dialog.bash\n"
